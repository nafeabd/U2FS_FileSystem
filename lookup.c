...
struct inode *wrapfs_iget_new(struct super_block *sb,
			struct inode *lower_inode, int index)
{
	...
	/* initialize new inode */
	info = WRAPFS_I(inode);
	info->lower_inodes = kzalloc(2, GFP_KERNEL);
	inode->i_ino = lower_inode->i_ino;
	if (!igrab(lower_inode)) {
		err = -ESTALE;
		return ERR_PTR(err);
	}
	wrapfs_set_lower_inode_idx(inode, index, lower_inode);
	inode->i_version++;
	/* use different set of inode ops for symlinks & directories */
	if (S_ISDIR(lower_inode->i_mode))
		inode->i_op = &wrapfs_dir_iops;
		     ...
		     struct path *lower_path, int index)
{
	...
	lower_sb = wrapfs_lower_super_idx(sb, index);
	...
	if (!dentry->d_inode) {
		inode = wrapfs_iget_new(sb, lower_inode, index);
		d_add(dentry, inode);
		goto out;
	} else {
...
set_inode:
		lower_dentry = wrapfs_lower_dentry_idx(dentry, index);
		wrapfs_set_lower_inode_idx(dentry->d_inode, index,
...
static struct dentry *__wrapfs_lookup(struct dentry *dentry, int flags,
				      struct path *lower_parent_path, int index)
{
	int err = 0;
	...
	if (!err) {
		wrapfs_set_lower_path_idx(dentry, &lower_path, index);
		err = wrapfs_interpose(dentry, dentry->d_sb, &lower_path,
								index);
		if (err) /* path_put underlying path on error */
			wrapfs_put_reset_lower_path_idx(dentry, index);
		goto out;
}
	/*
			     ...
			     struct nameidata *nd)
{
	...
	for (bindex = 0; bindex <= 1; ++bindex) {
		wrapfs_get_lower_path_idx(parent, &lower_parent_path, bindex);

	/* allocate dentry private data.  We free it in ->d_release*/
	ret = __wrapfs_lookup(dentry, nd->flags, &lower_parent_path, bindex);
	tindex = bindex;
	if (IS_ERR(ret)) {
		++counter;
		if (bindex == 0) {
		...
		if (counter == 1) {
			if (tindex == 0)
				wrapfs_set_lower_inode_idx(dentry->d_inode, 1,
								NULL);
			else
				wrapfs_set_lower_inode_idx(dentry->d_inode, 0,
						NULL);
		}
out:
